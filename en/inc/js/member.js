...
var checkFormUserEmail_m = function( fg ){
	var _alert = $userEmail.next();
	_alert.show();
	var c = checkInputValue( $userEmail.val() , 'mail' );
	if( c > 0 ){
		_alert.find('div').attr('class','color_red').text( eval('member_msg.userEmail.m'+c) );
		if(fg){
			alert( eval('member_msg.userEmail.m'+c) );
...
$userPhone1.change(function(){
	checkFormUserphoen();
});
$userPhone2.focus(function(){
	$userPhone3.next().show();
}).blur(function(){
	checkFormUserphoen();
});
$userPhone3.focus(function(){
	$userPhone3.next().show();
}).blur(function(){
	checkFormUserphoen();
});