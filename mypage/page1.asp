...
Sub GetList()
	...
	with objCmd
		...
		.Parameters("@UserId").value  = session("userId")
		Set objRs = .Execute
	End with
	set objCmd = nothing
		...
		<div id="contant">
			...
			<table cellpadding="0" cellspacing="0" style="border-top:2px solid #4f4f4f;width:100%;font-size:12px;margin-bottom:10px;table-layout:fixed;">
				...
				<%for iLoop = 0 to cntNoti
					onclick = "location.href='page1_view.asp?" & PageParams & "&idx=" & arrNoti(NOTICE_Idx,iLoop) & "'"
				%>
				<tr style="height:40px;">
				...
				<%for iLoop = 0 to cntList
					onclick = "location.href='page1_view.asp?" & PageParams & "&idx=" & arrList(FI_Idx,iLoop) & "'"
			...
			<div style="text-align:right;margin-bottom:10px;">
				<button class="btn_bg" style="width:80px;" onclick="location.href='page1_write.asp?<%=PageParams%>'">글쓰기</button>
			</div>
