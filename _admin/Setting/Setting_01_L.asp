...
Sub GetList()
	...
	with objCmd
		...
		.CommandText      = "KETI_IOT_SETTTING_TIME_L"
...
function openPop(mode,idx){
	...
	for( i=0; i<24 ; i++){
		t_val = i < 10 ? "0"+i : i;
		t_txt += '<option value="'+t_val+'">'+i+'</option>';
	}
	var m_txt,m_val;
	for( i=0; i<60 ; i++){
		m_val = i < 10 ? "0"+i : i;
	...
	var html_txt = '' +
		...
		'<form name="fm" method="POST" action="Setting_01_P.asp">' +
		'<input type="hidden" name="actType" value="'+mode+'"><input type="hidden" name="idx" value="'+idx+'"><input type="hidden" name="pageNo" value="<%=pageNo%>">' +
			...
			'<div class="top_area">' +
				'<div class="title"><img src="../img/pop/title_setting_01.gif"></div>' +
			...
			'<div class="cont">' +
				'<table cellpadding=0 cellspacing=0 width=100%>'+
					...
					'</tr>'+
					'<tr>' +
						'<td class="line_box" align=right bgcolor="f0f0f0">시간</td>'+
						'<td class="line_box">'+
							'<select id="name_t" name="name_t">'+t_txt+'</select> 시 '+
							'<select id="name_m" name="name_m">'+m_txt+'</select> 분'+
						'</td>'+
					'</tr>'+
					'<tr>' +
			...
			success: function(xml){
				...
				if ($(xml).find("data").find("item").length > 0) {
					$(xml).find("data").find("item").each(function(idx) {
						...
						$('#ord').val( ord );
						$("#name_t > option[value = " + tmp_name[0] + "]").attr("selected", "ture");
						$("#name_m > option[value = " + tmp_name[1] + "]").attr("selected", "ture");
...
<table cellpadding=0 cellspacing=0 width="990" align=center border=0>
	<tr>
		...
		<form name="AdminForm" method="post" action="Setting_01_P.asp">
					...
					<td colspan=2>
						<table cellpadding=0 cellspacing=0 width="100%" >
							<tr height="30" align=center bgcolor="f0f0f0">
								...
								<td class="line_box" width="10%">시간</td>
