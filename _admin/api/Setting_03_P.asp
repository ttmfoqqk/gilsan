...
Dim ea        : ea        = IIF( Request.Form("ea")="",0,Request.Form("ea") )
Dim device    : device    = Trim( Request.Form("device") )
...
Sub Insert()
	...
	with objCmd
		...
		.CommandText      = "KETI_IOT_SETTTING_DEVICE_P"
		.Parameters("@actType").value   = actType
		.Parameters("@Idx").value       = Idx
		.Parameters("@Ord").value       = Ord
		.Parameters("@Name").value      = device
...
<html>
 ...
 <body>
  ...
  location.href="Setting_03_L.asp?pageNo=<%=pageNo%>"
  </script>
 </body>
</html>