...
Sub GetList()
	...
	with objCmd
		...
		.CommandText      = "KETI_IOT_SETTTING_DEVICE_L"
...
function openPop(mode,idx){
	...
	var t_txt;
	for( i=0; i<20 ; i++){
		t_txt += '<option value="'+i+'">'+i+'</option>';
	}
	t_txt += '<option value="999">999</option>';
	var html_txt = '' +
		
		'<div class="admin_popup" id="admin_popup">' +
		'<form name="fm" method="POST" action="Setting_03_P.asp">' +
		'<input type="hidden" name="actType" value="'+mode+'"><input type="hidden" name="idx" value="'+idx+'"><input type="hidden" name="pageNo" value="<%=pageNo%>">' +
			...
			'<div class="top_area">' +
				'<div class="title"><img src="../img/pop/title_Setting_03.gif"></div>' +
			...
			'<div class="cont">' +
				'<table cellpadding=0 cellspacing=0 width=100%>'+
					...
					'</tr>'+
					'<tr>' +
						'<td class="line_box" align=right bgcolor="f0f0f0">장비명</td>'+
						'<td class="line_box"><input type="text" id="device" name="device" class="input"></td>'+
					'</tr>'+
					'<tr>' +
						'<td class="line_box" align=right bgcolor="f0f0f0">수량</td>'+
						'<td class="line_box">'+
							'<select id="ea" name="ea">'+t_txt+'</select>'+
						'</td>'+
					'</tr>'+
					'<tr>' +
			...
			success: function(xml){
				...
				if ($(xml).find("data").find("item").length > 0) {
					$(xml).find("data").find("item").each(function(idx) {
						...
						$('#ord').val( ord );
						$('#device').val( name );
						$("#ea > option[value = " + ea + "]").attr("selected", "ture");
...
<table cellpadding=0 cellspacing=0 width="990" align=center border=0>
	<tr>
		...
		<form name="AdminForm" method="post" action="Setting_03_P.asp">
					...
					<td colspan=2>
						<table cellpadding=0 cellspacing=0 width="100%" >
							<tr height="30" align=center bgcolor="f0f0f0">
								...
								<td class="line_box" width="20%">장비</td>
								<td class="line_box" width="10%">수량</td>
							...
							<tr height="30" align=center>
								...
								<td class="line_box"><%=arrList(FI_Ea,iLoop)%></td>
