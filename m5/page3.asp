		...
		<div id="contant">
			...
			<table cellpadding="0" cellspacing="0" style="border-top:2px solid #4f4f4f;width:100%;font-size:12px;margin-bottom:10px;table-layout:fixed;">
				...
				<%for iLoop = 0 to cntNoti
					onclick = "location.href='page3_view.asp?" & PageParams & "&idx=" & arrNoti(NOTICE_Idx,iLoop) & "'"
				%>
				<tr style="height:40px;">
				...
				<%for iLoop = 0 to cntList
					onclick = "location.href='page3_view.asp?" & PageParams & "&idx=" & arrList(FI_Idx,iLoop) & "'"
				%>
				<tr style="height:40px;">
					<td onclick="<%=onclick%>" style="border-bottom:1px solid #4f4f4f;text-align:center;cursor:pointer;"><%=arrList(FI_rownum,iLoop)%></td>
					<td onclick="<%=onclick%>" style="border-bottom:1px solid #4f4f4f;padding-left:10px;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><%=arrList(FI_Title,iLoop)%></td>
